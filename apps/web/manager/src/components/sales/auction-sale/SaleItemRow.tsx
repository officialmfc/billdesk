...
import type { SaleItem } from "./state";
...
export const SaleItemRow: React.NamedExoticComponent<SaleItemRowProps> = memo(
  ({
    item,
    index,
    updateItem,
          ...
          <div className="flex items-start gap-1">
            ...
            <div className="flex flex-2 items-center gap-1">
              <div className="flex-1 min-w-0">
                <Input
                  ref={(el) => assignRef("product", item.id, el)}
                  value={item.productDescription}
                  onChange={(e) =>
                    updateItem(item.id, "productDescription", e.target.value)
                  }
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
                      handleProductEnter(item.id);
                    }
                  }}
                  placeholder="Product..."
