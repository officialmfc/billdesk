...
const saleFlow = getManagerSaleFlowDefinition("floor");
...
export default function FloorSaleScreen() {
  ...
  const handleSubmit = async () => {
    if (!isOnline) {
      ErrorHandler.showWarning("Floor sales require connectivity.");
      return;
    }
    ...
    if (!saleItems.length) {
      ErrorHandler.showWarning("Add at least one valid floor-sale line.");
      return;
    }

    try {
      setSubmitting(true);
      await rpcService.createFloorSale({
      ...
      ErrorHandler.showSuccess("Floor sale created.");
      router.replace("/sales");
    } catch (error) {
      ErrorHandler.handle(error, "Floor sale");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <SectionHeader
        ...
        description="MFC seller, buyer, stock batch, weight, rate"
        action={
      ...
      <SurfaceCard style={styles.summaryCard} contentStyle={styles.summaryContent}>
        ...
        <Text variant="bodySmall" style={styles.mutedText}>
          {items.length} floor lines
        </Text>
      </SurfaceCard>

      {items.length > 0 ? (
        <SaleItemsPreviewTable
          description="Live floor-sale lines"
          columns={[
            { key: "index", label: "#", flex: 0.45 },
            { key: "buyer", label: "Buyer", flex: 1.3 },
            { key: "seller", label: "Seller", flex: 1.25 },
            { key: "batch", label: "Batch", flex: 1.25 },
            { key: "weight", label: "Wt", flex: 0.9, align: "right" },
            { key: "rate", label: "Rate", flex: 0.9, align: "right" },
            { key: "total", label: "Total", flex: 1.1, align: "right" },
          ]}
