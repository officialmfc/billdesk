...
const USER_MOBILE_DEVICE_ID_KEY = "user_mobile_device_id";
const USER_MOBILE_DEVICE_LABEL_KEY = "user_mobile_device_label";

if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error("Missing Supabase environment variables for user app.");
}

const resolvedSupabaseUrl = supabaseUrl;
const resolvedSupabaseAnonKey = supabaseAnonKey;

export const USER_SUPABASE_SESSION_STORAGE_KEY = "user_mobile_auth_session";
const USER_LAST_OAUTH_REDIRECT_STORAGE_KEY = "user_mobile_last_oauth_redirect";
export const USER_MOBILE_OAUTH_CALLBACK_URL = "mfcuser://oauth-callback";
...
async function getHostedDeviceContext(): Promise<{ deviceId: string; deviceLabel: string }> {
  ...
  let deviceId = await storage.getItem(USER_MOBILE_DEVICE_ID_KEY);
  if (!deviceId) {
    deviceId = createDeviceId();
    await storage.setItem(USER_MOBILE_DEVICE_ID_KEY, deviceId);
  }

  let deviceLabel = await storage.getItem(USER_MOBILE_DEVICE_LABEL_KEY);
  if (!deviceLabel) {
    ...
    await storage.setItem(USER_MOBILE_DEVICE_LABEL_KEY, deviceLabel);
  }

  return { deviceId, deviceLabel };
}

export async function touchHostedUserDeviceLease(accessToken?: string): Promise<void> {
...
export async function clearPersistedSupabaseSession(): Promise<void> {
  const storage = getStorage();
  const keys = [
    USER_SUPABASE_SESSION_STORAGE_KEY,
...
export const supabase = createClient(resolvedSupabaseUrl, resolvedSupabaseAnonKey, {
  auth: {
    ...
    storageKey: USER_SUPABASE_SESSION_STORAGE_KEY,
  },
});
...
async function mutateSessionWithRetry(
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    if (session?.access_token && session.refresh_token) {
      console.info(`[UserAuth] ${label} stored session after retryable network error`);
      return;
    }

    if (attempt < 2) {
      console.info(`[UserAuth] ${label} retrying after transient network error`);
...
export async function signInWithGoogleOnUserMobile(): Promise<void> {
  await openHostedUserLogin();
}

export async function openHostedUserLogin(email?: string): Promise<void> {
...
export async function openHostedUserPasswordReset(email?: string): Promise<void> {
  const device = await getHostedDeviceContext();
...
export async function completeOAuthRedirect(url: string): Promise<boolean> {
  console.info("[UserAuth] received oauth redirect", { url });
  const normalizedUrl = unwrapExpoDevClientUrl(url);
  if (normalizedUrl !== url) {
    console.info("[UserAuth] normalized expo dev client url", { normalizedUrl });
  }

  const isCustomSchemeCallback = normalizedUrl.startsWith(USER_MOBILE_OAUTH_CALLBACK_URL);
  const isExpoRouteCallback = (() => {
    try {
      const parsed = new URL(normalizedUrl, "https://user.local");
  ...
  console.info("[UserAuth] parsed oauth params", {
  ...
  if (handoffId) {
    console.info("[UserAuth] exchanging login handoff");
    try {
      await exchangeHandoff(handoffId);
    } catch (error) {
      console.error("[UserAuth] handoff exchange failed", error);
  ...
  if (code) {
    console.info("[UserAuth] exchanging oauth code");
    try {
      await mutateSessionWithRetry("oauth code exchange", () =>
        supabase.auth.exchangeCodeForSession(code)
      );
    } catch (error) {
      console.error("[UserAuth] oauth code exchange failed", error);
      throw new Error(error instanceof Error ? error.message : "Could not complete sign-in.");
    }
    await storage.setItem(USER_LAST_OAUTH_REDIRECT_STORAGE_KEY, normalizedUrl);
    return true;
  ...
  if (accessToken && refreshToken) {
    console.info("[UserAuth] restoring session from tokens");
    try {
      ...
      console.error("[UserAuth] token restore failed", error);
